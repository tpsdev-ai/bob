...
- **ONE session factory — bob no longer spawns pi or builds pi argv.** Every session (`bob run`, the persistent runtime, the launcher, the mail consumer, onboarding, alignment, and interactive `bob launch`) now comes from a single factory built on pi's runtime-factory contract, so the effective policy — the role's ceiling intersected with `bob.yaml`, minus `exclude` and the resident exclusions — is what the session is created with, and the policy is REQUIRED in the type and at runtime. `bob launch` takes AT MOST ONE PROMPT and nothing else: every other argument is refused BY NAME, so a caller-controlled flag can no longer reach pi's parser (which reads flags after bob's, last wins — the widening this area keeps recovering from). (`test/shell/launch.test.ts`, `run-tool-allowlist.test.ts`)
- **A session's resources are built by bob, isolated.** Project trust is off, no configured packages are resolved or installed, and pi's discovery of user or project extensions, skills, prompt templates, themes and context files is disabled — as is the global `SYSTEM.md` / `APPEND_SYSTEM.md`. The only extensions that load are the capabilities declared in `bob.yaml`, and a reload re-reads exactly those. **Compatibility note: ambient pi extensions, skills, prompt templates, themes and packages no longer load for bob agents.** Add a capability by declaring it in `bob.yaml` and listing its tools within the role. (`test/shell/session.test.ts`)
- **The tool audit runs at creation, when the interactive mode binds extensions, and after every reload.** Every name in the effective policy must be active in the session, and no name may come from two sources (a pi built-in or a declared capability). A failed audit disposes the session and ends the process with the named error, rather than leaving a running session whose policy no longer holds — pi's TUI shows a reload error and carries on, so a throw is not enough. (`test/shell/session.test.ts`)
...
- **`bob doctor` no longer suggests a fix that would fail.** A resident agent whose allowlist names a tool the resident policy drops used to be told to set `tools.allowResidentShell: true` in `bob.yaml` — which the role ceiling refuses at load, because `bob.yaml` may only narrow the role. The hint now names `roles/<role>/role.json`, where the grant actually lives. (`test/shell/doctor.test.ts`)
