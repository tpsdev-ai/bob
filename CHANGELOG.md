...
- **README usage section matches the CLI.** The usage section no longer names commands or flags the CLI does not have: `bob serve` (there is no `serve` command — a run with no prompt *is* the persistent mode), `--interactive` (the CLI rejects it), and "system cron" (scheduling is the persistent `bob run` runtime firing bob.yaml `cron:` entries in-process, not an OS cron). A new `test/readme-commands.test.ts` fails whenever the README names a command the CLI rejects.
