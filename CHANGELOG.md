...
- **The task — or a persistent agent's standing contract — rides the SYSTEM PROMPT, and every agent response request is checked for it.** A `bob run` task, or a resident agent's role + duties, is appended to the session's system prompt as LITERAL text through the resource loader's `appendSystemPromptOverride` (never as an append-system-prompt *source*: pi reads a source string that names an existing path as that FILE), bounded by a cap the block's own heading states and cut with a visible `[truncated: N chars elided]` marker. pi rebuilds the system prompt at session creation, at bind, on every reload and on every tool change, and a context compaction rewrites only the message history — so a compaction can no longer erase the thing the run is working on. bob's guard is registered LAST on `before_provider_request`, after every declared capability: an agent response request whose system prompt does not carry the contract fails the turn exactly like a failed audit — the session is disposed, the process ends, and the reason (`contract_missing_from_system_prompt`) is named. That covers both loss paths a capability has (replacing the prompt in `before_agent_start`, or rewriting the outgoing payload in `before_provider_request`), neither of which the creation/bind/reload audit can see. A blank task is refused BEFORE the session starts. **The guarantee is stated for AGENT RESPONSE requests:** pi's own compaction and branch-summary calls carry pi's summarization prompt instead and are excluded by name. (`test/shell/system-prompt-contract.test.ts`, `system-prompt-contract-live.test.ts`, `run.test.ts`)
...
- **After a compaction bob sends ONE best-effort "what remains" note**, as a steer: the last plan the agent stated, or a generated note with `git status --short` and the recent tool calls. It is never load-bearing — a failed send is logged and nothing else happens, and no exit code depends on it. (`test/shell/compaction-contract.test.ts`)
...
- **Stated limits of the contract above.** The task is sent in the first user message AND in every agent request's system prompt, so it costs tokens per request (within a run the prompt is stable and a provider's prompt cache can cover it; it is not shared across runs). A failed best-effort "what remains" note can lose the plan it would have quoted — the TASK is not lost, it is in the system prompt. And the completion judge checks for a real final MESSAGE, not that the work it describes was actually done.
