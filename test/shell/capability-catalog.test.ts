...
describe("blessed catalog", () => {
  ...
  it("blesses discord as a self-referencing specifier, not a path", () => {
    // The defect this replaces: discord was blessed as a path relative to the
    // shell package, which resolves only inside a checkout. A specifier into
    // this package's own `exports` map resolves in a checkout AND in a
    // published install, and to the same built file in both.
    const spec = lookupCapability("discord")?.manifest.piPackage ?? "";
    expect(spec).toBe("@tpsdev-ai/bob/capabilities/discord");
  });

  it("blesses EVERY implemented capability through the exports map", () => {
    // The invariant, as a check rather than as prose a reviewer has to recall:
    // no blessed capability may reintroduce a path or an npm:/git: source.
    for (const [name, entry] of Object.entries(BLESSED_CATALOG)) {
      if (entry.notYetImplemented) continue;
      expect(entry.manifest.piPackage, name).toBe(`@tpsdev-ai/bob/capabilities/${name}`);
    }
  });
  ...
  it("blesses the flair capability as implemented", () => {
    ...
    expect(entry?.manifest.piPackage).toBe("@tpsdev-ai/bob/capabilities/flair");
  });
  ...
  it("blesses the fixture the same way as a real capability", () => {
    // It used to be blessed as an absolute path into examples/, which meant the
    // capability the loader tests leaned on hardest was the one capability that
    // never exercised the resolution path being tested.
    expect(BLESSED_CATALOG.fixture.manifest.piPackage).toBe("@tpsdev-ai/bob/capabilities/fixture");
  });
...
// The same proof for a REAL capability rather than the fixture. flair is chosen
// because it opens nothing (no gateway, no network at load); discord and
// observatory take the identical path through the catalog.
describe("capability mechanism — a real capability, end to end", () => {
  it("resolves flair through the exports map and composes its tools", async () => {
    const yaml = [
      "capabilities:",
      "  - flair",
      "",
    ...
    // Resolved, not blessed-as-written: an absolute path to the SHIPPED entry
    // point, which is what `exports` points at in a checkout and in an install.
    expect(source.startsWith("/")).toBe(true);
    expect(source.endsWith("/dist/capabilities/flair/index.js")).toBe(true);
